...
<?php
	...
	$isReadOnly = false;
        // 2018/06/29 ↓
	if($filename == 'RIREKILIST_2' )
        {
		header("location:".(empty($_SERVER['HTTPS'])? "http://" : "https://")
				.$_SERVER['HTTP_HOST'].dirname($_SERVER["REQUEST_URI"])."/hanNyushutuList.php");
                exit();
        }
        // 2018/06/29 ↑
	...
	if($isexist)
	{
		...
		else
		{
			$data = "";
                ...
                {
                    $form = makeformZAIKOEdit_set($_SESSION['edit'],$out_column,$isReadOnly,"edit",$data );
