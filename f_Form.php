...
function makeformInsert_set($post,$out_column,$isReadOnly,$formName){
	...
	$error ="";
	$ReadOnlyBak ="";
	$EMcomp_array = array("現場名","顧客名","差異理由","資材名");				//フォームテキスト入力時全角指定文字列
	$form_input_type = "";									//全角入力スタイルシート追加用
                ...
                //---------------------------------------------------入力テキスト作成---------------------------------------------------------------------//
                           ...
                           //--------------------------------------↓2018/09/12 土場在庫追加---------------------------------------------------//
                                            ...
                                            if($Colum == 407)
                                            {
                                                ...
                                                $con = dbconect();
                                                $sql = "select * from dobainfo where DELETED = 0";
                                                 // クエリ発行
                                                $result = $con->query($sql);
                                                $insert_str .= '<select name="10CODE" class="doba">';
                                                //土場情報一覧
                                                ...
                                                {  
                                                    ...
                                                    {    
                                                        ...
                                                        if($search == $result_row['10CODE'])
                                                        {
                                                           $select = "selected";//プルダウン選択固定
                                                        }
                                                        
                                                    }

                                                    $insert_str .= "<option value=".$result_row['10CODE']." $select >".$result_row['DOBANAME']."</option>";
                                                }
						...
						else
						{
							...
							if(in_array($form_item_name,$EMcomp_array))
							{
								if(strpos($ReadOnly, 'readOnly') !==false)
								{
									$ReadOnly = "readOnly txtmode1";
								}
								else
								{
									$ReadOnly = "txtmode1";
...
function makeformSerch_set($post,$formName){
	...
	$readOnly = '';
	$hidden_value ="";
	$EMcomp_array = array("現場名","顧客名","差異理由","資材名");
	$form_input_type = "";
        // 2018/06/29 追加
        $makeDatepicker = "";
	...
	for($i = 0 ; $i < count($columns_array) ; $i++)
	{
		...
		if(isset($form_ini[$Colum]['table_name']))
		{
			...
			{
				$insertColumn = '1003,402,403,202,203';
			...
			$insertColumn_array = explode(',',$insertColumn);
                        // 2018/06/29 ↓ 追加修正 [選択]ボタン条件追加
			if($tablenum != $Colum  || $readonlyHeader== true )
			{
				$table_title = $form_ini[$Colum]['table_title'];
                                if( $readonlyHeader== false )
                                {
                    //------------------------------------↓2018/09/14 土場在庫追加-----------------------------------------//
                                    if($tablenum == 5 || $tablenum == 6 || $tablenum == 7 || $tablenum == 8 || $filename == "SIZAILIST_2")
                                    {
                                        $serch_str .= "<tr><td></td><td>";
                                        $serch_str .= '<input type="button" value="'.$table_title.'選択" 
                                                onclick="popup_modaldoba(\''.$columns_array[$i].'\')">';
                                        $serch_str .= '</td></tr>';
				...
				if($form_format_type == 9)
				{
					...
					for($k = 0 ; $k < $form_num ; $k++)
					{
						...
						$serch_str .= $form_delimiter.'<input type ="'.$input_type.'" name = "'
										...
										'"'.$readOnly.' size = "'.$form_size.'" '.$check_js.$form_input_type.' >';
					}
				}
				else if($form_format_type > 9)
				{
                ...
                //--------------------------------------------------↓2018/09/13 土場在庫追加---------------------------------------------------------------//                       
                                        if($Colum == 1005 || $Colum == 1004)
                                        {
                                                ...
                                                $sql = "select * from dobainfo where DELETED = 0";
                                                 // クエリ発行
                                                $result = $con->query($sql);
                                                $serch_str .= '<select name="doba" id ="DOBA" class="doba">';
                                                if($filename == "RIREKILIST_2" || $filename == "SIZAILIST_2")
                                                {
                                                    $serch_str .= "<option></option>";
                                                }    
                                                ...
                                                {  
                                                    ...
                                                    $serch_str .= "<option value=".$result_row['10CODE']." $select >".$result_row['DOBANAME']."</option>";
                                                }
            ...
            //--------------------------------------------------↓2018/09/19 土場在庫追加---------------------------------------------------------------//                       
                                if($Colum == 1003 ||$Colum == 1004 )
                                {
                                        ...
                                        {    
                                                ...
                                                // クエリ発行
                                                $result = $con->query($sql);
                                                $form_str .= '<select name="doba" id ="DOBA" class="doba">';
                                                ...
                                                {  
                                                    ...
                                                    $form_str .= "<option value=".$result_row['10CODE']." $select >".$result_row['DOBANAME']."</option>";
                                                }
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                    ...
                                                    if(in_array($form_item_name,$EMcomp_array))
                                                    {
                                                            if(strpos($isReadOnly, 'readOnly') !==false)
                                                            {
                                                                    $isReadOnly = "readOnly txtmode1";
                                                            }
                                                            else
                                                            {
                                                                    $isReadOnly = "txtmode1";
		...
		for ($list_count = 0 ; $list_count < $list_loop ; $list_count++)
		{
			for($j = 0 ; $j < $loop_count ; $j++)
			{
				...
				$pre_insert_str .= "<td class = 'two'><a class = 'comp' >";
                                ...
                                if($Colum != 407)//<--2018/09/12 土場在庫追加-----
                                {    
                                    for($k = 0 ; $k < 5 ; $k++)
                                    {
                                ...
                                {
                                    //----↓2018/09/12 土場在庫追加-----
                                    $dobaname = $_SESSION['insert']['10CODE'];
                                    $con = dbconect();
                                    $sql = "select * from dobainfo where 10CODE = '$dobaname'";
                                    // クエリ発行
                                    $result = $con->query($sql);
                                    if($result_row = $result->fetch_array(MYSQLI_ASSOC))
                                    {  
                                           $value = $result_row['DOBANAME'];
                                    }
                                    //----↑2018/09/12 土場在庫追加-----
                                }    
               ...
               //---------------------------------------------------↓2018/09/20 土場在庫追加----------------------------------------------------------------//                           
                                                if($Colum == 407)
                                                {
                                                    ...
                                                    $sql = "select * from dobainfo where DELETED = 0";
                                                     // クエリ発行
                                                    $result = $con->query($sql);
                                                    $insert_str .= '<select name="DOBA" class = "doba">';
                                                    //土場情報一覧
                                                    while($result_row = $result->fetch_array(MYSQLI_ASSOC))
                                                    {  
                                                        ...
                                                        {
                                                            $search = $_SESSION['edit']['DOBA'];
                                                            if($search == $result_row['10CODE'])
                                                            {
                                                               $select = "selected";//プルダウン選択固定
                                                            }
                                                        }
                                                        else if(isset($post['10CODE']))
                                                        {
                                                            $search = $post['10CODE'];
...
function EditComp($post,$data){
	...
	$form_ini = parse_ini_file('./ini/form.ini', true);							// form.ini呼び出し
	require_once 'f_Form.php';                                                                          	// f_From関数呼び出し
	$SQL_ini = parse_ini_file('./ini/SQL.ini', true);																						// DB関数呼び出し準備
...
//					}
        ...
        {
            $columnname_array = array("土場名","在庫数","土場在庫");
            
             $insert_str .= "<table class ='list' align = 'center'><tbody>";
             for($i = 0 ; $i < count($columnname_array) ; $i++)
             {       
                    $insert_str .="<th><a class ='head'>".$columnname_array[$i]."</a></th>";
             }
                                   
            $sql = $SQL_ini[$filename]['sql2'];                       
            $con = dbconect();
            $sql .= " AND SIZAIID = ".$post['form_102_0']."";
            $list_zaiko = $con->query( $sql);
            $num = 0;
            while($list_zaiko_row = $list_zaiko->fetch_array(MYSQLI_ASSOC))
            {
                $insert_str .="<tr>";
                $insert_str .= "<td>$list_zaiko_row[DOBANAME]</td>";                           
                $insert_str .= "<td>$list_zaiko_row[ZAIKONUM]</td>";                             
                $insert_str .= "<td>$list_zaiko_row[ZAIKO]</td>";                           
                //$insert_str .= "<td>$list_zaiko_row[SYUKKASUM]</td>";                           
                //$insert_str .= "<td>$list_zaiko_row[HENKYAKUSUM]</td>";                           
                $insert_str .= "</tr>";                                                                   
                ...
                $total_sql = $SQL_ini[$filename]['sql'].$SQL_ini[$filename]['sql1'];
                $total_sql .= " HAVING SIZAIID = ".$post['form_102_0']."";
                $total = $con->query( $total_sql);
                                     
            while($total_row = $total->fetch_array(MYSQLI_ASSOC))
            {

                $insert_str .="<tr class='border'>";
                $insert_str .= "<td>計</td>";
                $insert_str .= "<td>$total_row[ZAIKONUM]</td>";
                $insert_str .= "<td>$total_row[ZAIKO]</td>";
                //$insert_str .= "<td>$total_row[SYUKKASUM]</td>";
                //$insert_str .= "<td>$total_row[HENKYAKUSUM]</td>";
                $insert_str .= "</tr>";
                                           
            }
                 $insert_str .= "</tbody></table>";
            //--------------------------土場追加対応----------------------------------------------------------------------------------------------------//
        }
         //---------------------------↑2018/10/02 土場追加対応----------------------------------------------//
...
// 2018/06/29 追加対応 ↑
/************************************************************************************************************
function makeformZAIKOEdit_set($post,$out_column,$isReadOnly,$formName,$data)
...
function makeformZAIKOEdit_set($post,$out_column,$isReadOnly,$formName,$data){
	...
	$readonly_back = "";
	$EMcomp_array = array("現場名","顧客名","差異理由","資材名");
	$columnname_array = array("土場名","在庫数","土場在庫","出荷数","返却数");
			...
			for($j = 0 ; $j < $loop_count ; $j++)
			{
				...
				else
				{
                                    ...
                                    if($table_columns == "303")
                                    {
                                        ...
                                        //$insert_str .= "<th><a class ='head'></a></th>" ;
                                    }
                                    else
                                    {
                                        $insert_str .="<tr><td class = 'space'></td><td class ='one'>";
                ...
                //-------------------------2018/10/09 -土場追加対応----------------------------------------------------------------------------------------------------//                
                                if($table_columns == "303")
                                {
                                     ...
                                     $num = 0;
                                        while($list_zaiko_row = $list_zaiko->fetch_array(MYSQLI_ASSOC))
                                        {
                                           ...
                                           if(isset($post["zaiko$num"]))
                                           {
                                               ...
                                               {
                                                    $zaiko = $post["zaiko$num"];
                                               }    
                                               
                                               $insert_str .= "<td><input type='number'  id ='zaikonum' name='zaiko$num' class='zaiko' value='$zaiko'onKeyUp = 'total()' )\"></td>";
                                           }
                                           else
                                           {
                                               $insert_str .= "<td><input type='number'  id ='zaikonum' name='zaiko$num' class='zaiko' value='$list_zaiko_row[ZAIKONUM]'onKeyUp = 'total()'></td>";
                                           }    
                                           $insert_str .= "<input type='hidden' name='zaikocode$num' class='zaiko' value=".$list_zaiko_row['3CODE'].">";
                                           $insert_str .= "<td><input type='text' id='dobazaiko' name='doba$num' class='zaikochange' value='$list_zaiko_row[ZAIKO]' readonly></td>";
                                           $insert_str .= "<td><input type='text' id='syukkanum' name='syukka$num' class='zaikochange' value='$list_zaiko_row[SYUKKASUM]' readonly></td>";
                                           $insert_str .= "<td><input type='text' id='henkyakunum' name='henkyaku$num' class='zaikochange' value='$list_zaiko_row[HENKYAKUSUM]' readonly></td>";
                                           //$insert_str .= "<td>$list_zaiko_row[ZAIKO]</td>";
                                           //$insert_str .= "<td>$list_zaiko_row[SYUKKASUM]</td>";
                                           //$insert_str .= "<td>$list_zaiko_row[HENKYAKUSUM]</td>";                                        
                                           $insert_str .= "</tr>";
                                     ...
                                     $insert_str .= "<input type='hidden' id ='zaikonumber' name='number' class='zaiko' value=".$num.">";
                                     $total_sql = $SQL_ini[$filename]['sql'].$SQL_ini[$filename]['sql1'];
                                     $total_sql .= " HAVING SIZAIID = ".$post['form_102_0']."";
                                     $total = $con->query( $total_sql);
                                    ...
                                    while($total_row = $total->fetch_array(MYSQLI_ASSOC))
                                    {
                                           ...
                                           $insert_str .= "<td>計</td>";
                                           //$insert_str .= "<td>$total_row[ZAIKONUM]</td>";
                                           $insert_str .= "<td><input type='text' id='zaikototal' class='zaikochange' ></td>";
                                           $insert_str .= "<td><input type='text' id='dobatotal' class='zaikochange' value='$total_row[ZAIKO]' readonly></td>";
                                           //$insert_str .= "<td>$total_row[ZAIKO]</td>";
                                           $insert_str .= "<td align='right'>$total_row[SYUKKASUM]</td>";
                                           $insert_str .= "<td align='right'>$total_row[HENKYAKUSUM]</td>";
                                           $insert_str .= "</tr>";
